-- Initial Supabase schema for the Magic: The Gathering Commander deck tracker.
-- This migration avoids destructive drops so it is safe for a GitHub-linked project.
...
CREATE TABLE IF NOT EXISTS color_identities (
...
CREATE INDEX IF NOT EXISTS idx_decks_color_identity_id ON decks(color_identity_id);
...
-- One row per deck color, used by statistics such as "most used color".
CREATE OR REPLACE VIEW deck_individual_colors AS
